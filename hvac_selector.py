...
def select_equipment(zones: list, outdoor_db: float, outdoor_wb: float,
    ...
    for zone in zones:
        ...
        # ── If no in-bounds candidates, pick the closest OOB unit ────────
        best_oob = None
        if not candidates and _oob_candidates:
            target = (tc_min + tc_max) / 2.0
            best_oob_raw = min(_oob_candidates, key=lambda c: abs(c["tc_sys"] - target))
            c = best_oob_raw
            htg_cap_oob = None
            heating_ok_oob = True
            if c["odu_series"] in [HP_SINGLE, HP_TWO] and htg_load > 0:
                try:
                    _, htg_integ, _ = get_hp_heating_cap(
                        c["odu_model"], c["odu_series"], c["cfm"],
                        entering_db, outdoor_db, "High"
                    )
                    htg_cap_oob = htg_integ
                    if not math.isnan(htg_cap_oob) and htg_cap_oob < htg_load:
                        heating_ok_oob = False
                except Exception:
                    pass
            kw_c = c.get("kw_sys")
            eer_c = round((c["tc_sys"] * 1000) / (kw_c * 1000), 2) if kw_c and not math.isnan(kw_c) and kw_c > 0 else None
            best_oob = {
                "zone": zone_name,
                "odu_model": c["odu_model"],
                "odu_series": c["odu_series"],
                "nom_tons": c["nom_tons"],
                "nom_kbtu": c["nom_kbtu"],
                "fc_series": c["fc_series"],
                "idu_pattern": c["idu_pattern"],
                "cfm": c["cfm"],
                "tc_sys_kbtu": round(c["tc_sys"], 2),
                "shc_sys_kbtu": round(c["shc_sys"], 2),
                "tc_load_kbtu": tc_load,
                "shc_load_kbtu": shc_load,
                "tc_pct": round(c["tc_sys"] / tc_load * 100, 1),
                "sst_f": round(c["sst"], 1) if c.get("sst") else None,
                "sdt_f": round(c["sdt"], 1) if c.get("sdt") else None,
                "kw_odu": round(kw_c, 2) if kw_c and not math.isnan(kw_c) else None,
                "eer_site": eer_c,
                "htg_cap_kbtu": round(htg_cap_oob, 2) if htg_cap_oob and not math.isnan(htg_cap_oob) else None,
                "htg_load_kbtu": htg_load,
                "htg_ok": heating_ok_oob,
                "outdoor_db": outdoor_db,
                "outdoor_wb": outdoor_wb,
                "entering_ewb": round(indoor_ewb, 1),
                "oob": True,
                "oob_pct_diff": round(
                    (c["tc_sys"] / tc_load * 100) - cap_max_pct
                    if c["tc_sys"] > tc_max
                    else cap_min_pct - (c["tc_sys"] / tc_load * 100),
                    1
                ),
            }
        ...
        results.append({
            ...
            "tc_load": tc_load,
            "shc_load": shc_load,
            "selected": candidates[0] if candidates else best_oob,
            "out_of_bounds": best_oob is not None and not candidates,
...
# ─────────────────────────────────────────────────────────────────────────────
# MULTI-TYPE SELECTION (AC + HP side by side)
# ─────────────────────────────────────────────────────────────────────────────

def select_equipment_multi(zones, outdoor_db, outdoor_wb,
                            entering_db=80.0, entering_ewb=67.0,
                            cap_min_pct=100.0, cap_max_pct=115.0,
                            ac_types=None, hp_types=None):
    """Run AC and HP selections separately and return both per zone.

    Returns list of dicts with keys:
        zone, tc_load, shc_load,
        ac, ac_out_of_bounds, ac_all_candidates,
        hp, hp_out_of_bounds, hp_all_candidates
    """
    ac_types = ac_types or [AC_SINGLE, AC_TWO]
    hp_types = hp_types or [HP_SINGLE, HP_TWO]

    ac_results = select_equipment(
        zones, outdoor_db, outdoor_wb, entering_db, entering_ewb,
        cap_min_pct, cap_max_pct, equipment_types=ac_types,
    )
    hp_results = select_equipment(
        zones, outdoor_db, outdoor_wb, entering_db, entering_ewb,
        cap_min_pct, cap_max_pct, equipment_types=hp_types,
    )

    combined = []
    for ac_r, hp_r in zip(ac_results, hp_results):
        combined.append({
            "zone":              ac_r["zone"],
            "tc_load":           ac_r["tc_load"],
            "shc_load":          ac_r["shc_load"],
            "ac":                ac_r["selected"],
            "ac_out_of_bounds":  ac_r.get("out_of_bounds", False),
            "ac_all_candidates": ac_r.get("all_candidates", []),
            "hp":                hp_r["selected"],
            "hp_out_of_bounds":  hp_r.get("out_of_bounds", False),
            "hp_all_candidates": hp_r.get("all_candidates", []),
        })
    return combined
