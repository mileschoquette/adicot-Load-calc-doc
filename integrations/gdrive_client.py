...
def get_submit_folder(job_no: str,
    ...
    """Resolve the 6-Submit folder for a job.

    If `folder_id` is given it is used as the job folder directly (a manually
    chosen Drive folder), bypassing the Job-No name walk — this keeps reads and
    writes pointed at the same place the engineer picked. Otherwise the job
    folder is resolved by walking 1-job/{company}/{job_no}.
    ...
    job_folder = folder_id or (_walk_to_job(service, job_no) if job_no else None)
    if not job_folder:
        return None

    if create_if_missing:
        submit_id = _ensure_folder(service, job_folder, _SUBMIT_FOLDER_NAME)
    else:
        submit_id = _resolve_folder(service, job_folder, _SUBMIT_FOLDER_NAME)
    if not submit_id:
        return None
...
def list_submit_files(job_no: str, folder_id: Optional[str] = None) -> list[dict]:
    """List files in the job's 6-Submit folder as [{id, name, mimeType}].
    `folder_id` (the chosen job folder) overrides the Job-No walk. [] on failure."""
    service = _build_service()
    if service is None:
        return []
    submit = get_submit_folder(job_no, create_if_missing=False, folder_id=folder_id)
    if not submit:
        return []
    try:
        resp = service.files().list(
...
def _first_html_child(service, parent_id: str) -> Optional[dict]:
    ...
    Lists the folder's files and filters by extension in Python — Drive's
    `name contains` operator does NOT reliably match file extensions like
    '.html' (it matches word prefixes, not arbitrary substrings), which made
    the manual-folder search miss obvious dm_hvac-loads1.html files.
    """
    if not parent_id:
        return None
    try:
        resp = service.files().list(
            q=f"'{parent_id}' in parents and trashed = false "
              f"and mimeType != '{_FOLDER_MIME}'",
            fields="files(id, name)", pageSize=400, spaces="drive",
        ...
        return None
    files = resp.get("files", [])
    for f in files:                       # exact DM filename wins
        if f.get("name") == _HTML_FILENAME:
            return f
    for f in files:                       # else any .html / .htm
...
def _parent_id(service, folder_id: str) -> Optional[str]:
    """The id of a folder's parent (first parent), or None."""
    try:
        meta = service.files().get(fileId=folder_id, fields="parents",
                                   supportsAllDrives=True).execute()
        parents = meta.get("parents") or []
        return parents[0] if parents else None
    except Exception as e:
        log.error("Drive parent lookup for %s failed: %s", folder_id, e)
        return None


def resolve_job_folder(folder_id: str) -> str:
    """Normalize a manually-picked folder to the JOB folder — the one that holds
    both 4-Design and 6-Submit. If the engineer picked the 4-Design (or 6-Submit)
    subfolder, climb to its parent so reads (4-Design) and writes (6-Submit) stay
    consistent. Falls back to the picked folder when the structure isn't found."""
    service = _build_service()
    if service is None or not folder_id:
        return folder_id
    # Already a job folder? (contains a 4-Design or 6-Submit child)
    for sub in (_DESIGN_FOLDER_NAME, _SUBMIT_FOLDER_NAME):
        if _resolve_folder(service, folder_id, sub):
            return folder_id
    # Otherwise climb one level if the parent looks like the job folder.
    parent = _parent_id(service, folder_id)
    if parent:
        for sub in (_DESIGN_FOLDER_NAME, _SUBMIT_FOLDER_NAME):
            if _resolve_folder(service, parent, sub):
                return parent
    return folder_id
...
def upload_files(job_no: str,
    ...
    """Upload one or more files to the job's 6-Submit folder.

    `folder_id` (a manually chosen job folder) overrides the Job-No name walk, so
    PDFs land in the same folder the engineer picked for reading the HTML.
    ...
    submit = get_submit_folder(job_no, create_if_missing=True, folder_id=folder_id)
    if not submit:
        result["errors"].append({
            "name": None,
            "stage": "folder",
