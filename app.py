...
_INVOICE_REGISTRY = JOBS_DIR / "qbo_invoices.json"


def _load_invoice_registry() -> dict:
    try:
        return json.loads(_INVOICE_REGISTRY.read_text())
    except (FileNotFoundError, json.JSONDecodeError):
        return {}


def _save_invoice_registry(reg: dict) -> None:
    _INVOICE_REGISTRY.parent.mkdir(parents=True, exist_ok=True)
    tmp = _INVOICE_REGISTRY.with_suffix(".tmp")
    tmp.write_text(json.dumps(reg, indent=2))
    tmp.replace(_INVOICE_REGISTRY)
...
def create_invoice_route(wix_id: str):
    ...
    rec = {
        ...
        "url":         qbo.invoice_url(result["invoice_id"]),
        "job_no":      job_no,
        "customer_id": customer_id,
    }
    reg[wix_id] = rec
    _save_invoice_registry(reg)
    return jsonify({"ok": True, "invoice": rec})
