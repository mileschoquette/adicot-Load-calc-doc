...
def job_equip_run(job_id: str):
    ...
    if not HAS_EQUIP_SELECTOR:
        flash("hvac_selector.py or equipment_db.xlsx not found — equipment selection unavailable.")
    ...
    try:
        equipment_types = _EQUIP_TYPE_MAP.get(eq_type)
        results = eng.select_equipment(
    ...
    # Flag HP heating data gaps
    hp_series = {"GH5SAN5", "GH8TAN5"}
    for res in results:
        sel = res.get("selected")
        res["htg_data_missing"] = bool(
            sel
            and sel.get("odu_series") in hp_series
    ...
    try:
        eng.write_excel_schedule(
            results, str(xlsx_path), project_name, odb, owb, cap_min, cap_max
        )
    except Exception:
        traceback.print_exc()
        xlsx_name = None
