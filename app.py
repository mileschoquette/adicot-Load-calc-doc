...
def upload():
    ...
    # ── Snapshot the Wix record at job-run time, so the validator (Phase 3) ──
    # can compare HTML against what Wix said WHEN THIS JOB RAN, not against
    # whatever Wix says now if the project gets edited later.
    wix_snapshot = None
    if wix_item_id:
        wix_snapshot = wix_client.get_project(wix_item_id)
        if wix_snapshot is None:
            # Linked to a project but couldn't fetch it — log and continue.
    ...
    meta = {
        ...
        # mismatches: populated in Phase 3 once validators.py is wired in
        "mismatches":      [],
    }
