...
def _drive_submit_pdfs(job_no: str) -> list[dict]:
    """PDFs in the job's Google Drive 6-Submit folder as [{id, name}]."""
    pdfs = []
    for f in gdrive_client.list_submit_files(job_no):
        name = (f.get("name") or "")
...
def _attach_drive_pdfs(invoice_id: str, job_no: str, file_ids: list[str]):
    ...
    index = {f["id"]: f["name"] for f in _drive_submit_pdfs(job_no) if f.get("id")}
    attached, errors = [], []
    for fid in file_ids:
        name = index.get(fid)
...
def api_qbo_prepare(wix_id: str):
    ...
    return jsonify({
        ...
        "pdfs": _drive_submit_pdfs(job_no),
    })
...
def create_invoice_route(wix_id: str):
    ...
    attached, attach_errors = _attach_drive_pdfs(invoice_id, job_no,
                                                 request.form.getlist("pdfs"))
...
def attach_to_invoice_route(wix_id: str):
    ...
    attached, attach_errors = _attach_drive_pdfs(rec["invoice_id"], job_no, selected)
