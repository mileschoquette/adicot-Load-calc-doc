...
def _project_pdfs(wix_id: str) -> list[str]:
    """PDF filenames generated for a CMS project (its job dir is keyed by wix_id)."""
    safe = secure_filename(wix_id)
    if not safe or safe != wix_id:
        return []
    out = JOBS_DIR / safe / "out"
    if not out.exists():
        return []
...
def create_invoice_route(wix_id: str):
    ...
    # Attach selected PDFs (best-effort — a failed attach never undoes the invoice).
    attached, attach_errors = [], []
    available = set(_project_pdfs(wix_id))
    out_dir = JOBS_DIR / secure_filename(wix_id) / "out"
    for name in request.form.getlist("pdfs"):
        if name not in available:           # whitelist guard (no traversal)
            continue
        try:
            data = (out_dir / name).read_bytes()
        except OSError as e:
            attach_errors.append({"name": name, "error": str(e)})
            continue
        res = qbo.attach_file(invoice_id, name, data)
        if res.get("ok"):
            attached.append(name)
        else:
            attach_errors.append({"name": name, "error": res.get("error")})
...
def attach_to_invoice_route(wix_id: str):
    ...
    for name in selected:
        ...
        try:
            ...
            attach_errors.append({"name": name, "error": str(e)})
            continue
        res = qbo.attach_file(rec["invoice_id"], name, data)
        if res.get("ok"):
            attached.append(name)
        else:
            attach_errors.append({"name": name, "error": res.get("error")})
