...
Routes (URL → view function name):
    ...
    /job/<job_id>/spec/save          job_spec_save   POST — save spec inputs
    /job/<job_id>/spec/save-text     job_spec_save_text POST — save inline clause edits
    /job/<job_id>/spec/dxf           job_spec_dxf    POST — generate AutoCAD DXF
    /job/<job_id>/spec/docx          job_spec_docx   POST — generate Word Doc + Drive push
...
def _apply_text_overrides(rendered_spec, overrides: dict):
    """Apply saved inline clause text edits to a RenderedSpec in place."""
    if not overrides:
        return
    for part in rendered_spec.parts:
        for section in part.sections:
            for clause in section.clauses:
                key = f"{section.num}__{clause.label.replace('.', '')}"
                if key in overrides:
                    clause.text = overrides[key]
...
def job_spec(job_id: str):
    ...
    spec = spec_engine.build_spec(data, ctx, include_notes=True)

    # Apply saved inline text overrides so preview reflects engineer edits
    _apply_text_overrides(spec, meta.get("spec_text_overrides", {}))

    # Collect any already-generated spec output files for download links
    job_dir = _job_dir(job_id)
    out_dir = job_dir / "out"
    spec_files = []
    if out_dir.exists():
        for ext in (".dxf", ".docx"):
            for f in sorted(out_dir.glob(f"*-Specifications{ext}")):
...
@app.route("/job/<job_id>/spec/save-text", methods=["POST"])
@_require_auth
def job_spec_save_text(job_id: str):
    """Save engineer's inline clause text edits into meta['spec_text_overrides'].

    Form field names: clause_text__<section_num>__<label_no_dot>
    e.g. clause_text__1.01__A  ->  stored as  1.01__A
    """
    _job_dir(job_id)
    meta = _load_meta(job_id)
    ...
    for key, val in request.form.items():
        if key.startswith("clause_text__"):
            overrides[key[len("clause_text__"):]] = val.strip()

    meta["spec_text_overrides"] = overrides
    _save_meta(job_id, meta)
    flash("Spec text saved.")
    return redirect(url_for("job_spec", job_id=job_id))
...
def job_spec_dxf(job_id: str):
    """Generate the spec .dxf (notes stripped) into out/."""
    job_dir = _job_dir(job_id)
    meta = _load_meta(job_id)
    out_dir = job_dir / "out"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    _apply_text_overrides(rendered, meta.get("spec_text_overrides", {}))
    ...
    try:
        ...
        flash(f"DXF generated: {out_path.name}")
    except Exception as e:
        flash(f"DXF generation failed: {e}")

    return redirect(url_for("job_spec", job_id=job_id))


@app.route("/job/<job_id>/spec/docx", methods=["POST"])
@_require_auth
def job_spec_docx(job_id: str):
    """Generate the spec .docx, save to out/, then push to Drive 6-Submit folder."""
    job_dir = _job_dir(job_id)
    meta = _load_meta(job_id)
    out_dir = job_dir / "out"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    project_name = meta.get("project_name", "Specification")
    safe = project_name.replace(" ", "_").replace("/", "-")
    out_path = out_dir / f"{safe}-Specifications.docx"
    ...
    if wix_job_no and gdrive_client._parse_company_from_job_no(wix_job_no):
        try:
            upload_result = gdrive_client.upload_files(
                wix_job_no,
                [(out_path.name, out_path.read_bytes(),
                  "application/vnd.openxmlformats-officedocument.wordprocessingml.document")]
            )
            drive_result = {
                "status": "success" if upload_result["ok"] else "error",
                "folder_url": upload_result.get("folder_url"),
            }
        except Exception as e:
            drive_result = {"status": "error", "reason": str(e)}

    if drive_result["status"] == "success":
        flash(f"Word doc generated and uploaded to Drive ({wix_job_no}/6-Submit).")
    elif drive_result["status"] == "skipped":
        flash(f"Word doc generated: {out_path.name}. No Wix project linked — use the download link below.")
    else:
        flash(f"Word doc generated but Drive upload failed. Use the download link below.")
