...
# The work order surfaces these Wix fields, in this order. Booleans render Yes/No.
_WORK_ORDER_FIELDS = [
    ("jobNo",         "Job No"),
    ...
    ("coolingEff",    "Cooling Eff (SEER2)"),
    ("manufacturer",  "Manufacturer"),
    ("acNewExisting", "Thermostat Scope"),
    ("hasOutsideAir", "Outside Air"),
    ("hasExhaust",    "Exhaust"),
    ("suspCeiling",   "Susp. Ceiling"),
    ("description",   "Scope / Description"),
]


def _work_order_rows(snapshot: Optional[dict]) -> list[dict]:
    snap = snapshot or {}
    rows = []
    for key, label in _WORK_ORDER_FIELDS:
        val = snap.get(key)
...
def job_star(job_id: str):
    ...
    wo = _work_order_rows(meta.get("wix_snapshot")) if source == "cms" else None

    return render_template(
        "job_star.html",
        active_tab="star", job_id=job_id, meta=meta,
        source=source, parsed=parsed, wo=wo,
    )
...
def generate_pdfs(job_id: str):
    ...
    # Build the combined PDF (3 deliverables + selected charts) before the Drive
    # push so it rides along with the *.pdf upload below.
    _rebuild_combined(job_dir, meta)
...
def _rebuild_combined(job_dir: Path, meta: dict) -> Optional[Path]:
    """(Re)build <prefix>-Combined.pdf from the three deliverables plus the
    charts selected in meta['combined_charts']. Returns the path or None.
    Never raises — combining is best-effort and must not break PDF generation."""
    out_dir = job_dir / "out"
    ...
    try:
        return pdf_combine.build_combined_pdf(out_dir, ordered)
    except Exception:
        traceback.print_exc()
        return None
...
def job_charts_select(job_id: str):
    ...
    if (combined and combined.exists() and wix_job_no
            and gdrive_client._parse_company_from_job_no(wix_job_no)):
        try:
...
def job_parse(job_id: str):
    ...
    if has_upload:
        ...
        if not job_no:
            flash("Couldn't look up the Wix project's Job No — upload the HTML manually.")
            return redirect(url_for("job_star", job_id=job_id))
        fetched = gdrive_client.find_html(job_no)
        if fetched is None:
            flash(f"Couldn't fetch the HTML from Drive for {job_no}. Upload it manually instead.")
            return redirect(url_for("job_star", job_id=job_id))
        drive_filename, drive_bytes = fetched
    else:
    ...
    try:
        toilet_exh = float(prev_cfg.get("toilet_exhaust_cfm") or 70)
    except (TypeError, ValueError):
        toilet_exh = 70.0
