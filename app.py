...
# ── TEMPORARY: Wix schema debugger ──
# DELETE THIS ROUTE once we've confirmed the Wix field structure.
# Hit /debug/wix-schema in the browser to see what Wix's API returns.
@app.route("/debug/wix-schema")
@_require_auth
def _debug_wix_schema():
    api_key = os.environ.get("WIX_API_KEY")
    site_id = os.environ.get("WIX_SITE_ID")
    if not api_key or not site_id:
        return jsonify({
            "error": "WIX_API_KEY or WIX_SITE_ID not set in environment",
            "WIX_API_KEY_set": bool(api_key),
            "WIX_SITE_ID_set": bool(site_id),
        }), 500
    ...
    # 1. List all data collections so we can see the exact Projects collection ID
    try:
        r = requests.get(
            "https://www.wixapis.com/wix-data/v2/collections",
            headers=headers, timeout=10,
        )
        out["collections_status"] = r.status_code
        try:
            out["collections"] = r.json()
        except ValueError:
            out["collections_raw"] = r.text[:2000]
    except Exception as e:
        out["collections_error"] = str(e)

    # 2. Try to query Projects (guessing the collection ID is "Projects")
    for guess_id in ("Projects", "projects"):
        try:
            r2 = requests.post(
                "https://www.wixapis.com/wix-data/v2/items/query",
                headers=headers,
                json={"dataCollectionId": guess_id,
                      "query": {"paging": {"limit": 1}}},
                timeout=10,
            )
            out[f"query_{guess_id}_status"] = r2.status_code
            try:
                out[f"query_{guess_id}"] = r2.json()
            except ValueError:
                out[f"query_{guess_id}_raw"] = r2.text[:2000]
            if r2.ok:
                break  # found one that works, no need to try the other
        except Exception as e:
            out[f"query_{guess_id}_error"] = str(e)
