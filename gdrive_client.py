...
The 1-Jobs folder MUST live on a Shared Drive (not personal My Drive).
...
def _find_one_jobs_root(service) -> Optional[str]:
    ...
    Searches allDrives so the service account can find the folder on a
    Shared Drive. Service accounts cannot write to personal My Drive and
    will get a storageQuotaExceeded 403 if the folder is not on a
    Shared Drive.
    """
    try:
        resp = service.files().list(
    ...
    if not root_files:
        log.error("'%s' folder not visible to service account. "
                  "Verify it's on a Shared Drive and shared with the "
                  "service account email.", _ROOT_FOLDER_NAME)
        return None
    if len(root_files) > 1:
...
def find_html(job_no: str) -> Optional[tuple[str, bytes]]:
    ...
    Walks: Shared Drive → 1-Jobs → {company} → {job_no} → 4-Design → file
