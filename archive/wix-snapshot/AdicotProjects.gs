...
function _processIntakeThread(thread, processedLabel, intakeLabel) {
  ...
  var data = {
    ...
    skylightU:          merged.skylightU          || null,
    skylightSHGC:       merged.skylightSHGC       || null,
...
function _extractFromAttachment(b64, mediaType, filename) {
  try {
    ...
    var prompt = [
      ...
      '    Record skylightU/skylightSHGC only if the schedule states them outright',
...
function notifyProjectsSheet(data, sheetRowIndex) {
  try {
    ...
    var fields = {
      ...
      skylightU: data.skylightU || "",
      skylightSHGC: data.skylightSHGC || "",
