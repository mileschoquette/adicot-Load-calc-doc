...
const SHEET_COLUMNS = [
  ...
  // glassOperU..glassSGDSHGC and skylightU/skylightSHGC further down are retired
  // columns kept for positional stability; nothing writes them any more.
...
function _extractFromAttachment(b64, mediaType, filename) {
  try {
    ...
    var prompt = [
      ...
      '  - Glass U-factor (e.g. "U=0.28", "U-FACTOR: 0.35") — one value for all glass on the job (glassU/glassSHGC)',
      '  - Glass SHGC (e.g. "SHGC=0.25")',
      '  - NEVER infer a U-factor or SHGC from a glass description. If the drawings give only a',
      '    type (e.g. "SINGLE PANE CLEAR" or "INSULATED TINTED"), leave glassU and glassSHGC null',
      '    and record glassFrame / glazingType / glazingTint instead — the app derives the numbers',
      '    from IECC/FBC-EC Table C303.1.3, and a guessed value would silently outrank the code default.',
      '  - If the schedule distinguishes fixed vs operable vs sliding glass door, record the most common',
      '    (or worst-case) row in glassU/glassSHGC — the app carries one U/SHGC pair for all glass',
      '  - Glass frame (glassFrame): aluminum/steel -> "Metal"; thermally broken or thermally improved',
      '    aluminum -> "Metal with Thermal Break"; vinyl, wood, fiberglass, composite or clad ->',
      '    "Nonmetal or Metal Clad"; glass block -> "Glazed Block"',
      '  - Glazing (glazingType): "Single" for single/monolithic pane, "Double" for insulated/dual pane,',
      '    "Triple" for triple pane. Tint (glazingTint): "Clear" or "Tinted" — low-e or reflective coatings',
      '    are NOT a tint, so use the underlying clear/tinted state',
      '  - Skylights: note quantity/size/type if shown on the roof plan or window schedule (skylights)',
      '  - Opaque door type (doorType) and its U-factor (doorU) if the schedule states one.',
      '    If the building has no opaque exterior door (all glass storefront), record doorType "None"',
      '',
      'EQUIPMENT SCHEDULE:',
      ...
      '',
      '',
      'CONTROLLED VOCABULARY — these fields feed dropdowns in the client portal and',
      'the work order. Return one of the listed values EXACTLY as spelled, or leave',
      'the field blank. Do not invent a spelling: writing "metal" where the list says',
      '"Metal frame", or "gray" where it says "Medium", makes the value unusable.',
      'If the drawings show something genuinely outside a list, use "Other" where the',
      'list offers it, otherwise leave blank and mention it in description.',
      '(Kept in sync by hand with _FIELD_OPTIONS in blueprints/job_lifecycle.py.)',
      ...
      '  - suspCeiling: Suspended ACT (T-bar) | GWB | Open to structure | Conditioned space above — no roof load | Other',
      '  - atticCond: Sealed/conditioned plenum | Vented attic | None (flat roof) | Other',
      '  - wallFinish: Stucco | EIFS | Brick | Metal panel | Other',
      '  - wallConstruction: CMU | CMU + rigid insul | ICF | Steel stud + batt | Steel stud + rigid insul | Wood frame | Other',
      '  - wallColor: Light | Medium | Dark',
      '  - partConstruction: NA | Metal stud (steel frame) | Wood stud | Concrete Block / CMU',
      '  - floorType: Slab on grade | Floor over conditioned space | Floor over unconditioned space | Wood frame over unconditioned space | Elevated / exterior exposure',
      '  - glassFrame: Metal | Metal with Thermal Break | Nonmetal or Metal Clad | Glazed Block',
      '  - glazingType: Single | Double | Triple',
      '  - glazingTint: Clear | Tinted',
      '  - doorType: Uninsulated Metal | Insulated Metal (Rolling) | Insulated Metal (Other) | Wood | Insulated, nonmetal edge, max 45% glazing, any glazing double pane | None',
      '    (use None only when the drawings show no opaque exterior door at all)',
      '  - occupancyType: Dining / Fast food | Food prep / Kitchen | Office | Retail | Medical office | Assembly / Classrooms | Warehouse | Residential | Other',
...
function _mergeExtractions(emailData, pdfExtractions) {
  ...
  var TECHNICAL_FIELDS = ['sf','occupancyType','buildingStatus','occupants','orientation',
    ...
    'doorType','glassU','glassSHGC','doorU',
