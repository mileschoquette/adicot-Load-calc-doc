...
# ── OAuth endpoints via Intuit's discovery document ──────────────────
# Best practice: read the current OAuth endpoints from Intuit's discovery doc
# (cached 24h) instead of hardcoding, so they can't go stale. Falls back to the
# known-good constants above if the doc can't be fetched.
_DISCOVERY_URLS = {
...
def _discovery() -> dict:
    ...
    if cached and time.time() - cached["at"] < 24 * 3600:
        return cached["doc"]
    url = _DISCOVERY_URLS.get(env, _DISCOVERY_URLS["sandbox"])
    try:
        resp = requests.get(url, headers={"Accept": "application/json"},
                            timeout=_HTTP_TIMEOUT)
        if resp.ok:
            doc = resp.json() or {}
            _discovery_cache[env] = {"doc": doc, "at": time.time()}
            return doc
    except requests.RequestException as e:
        log.warning("QBO discovery fetch failed; using built-in endpoints: %s", e)
    return {}


def _auth_endpoint() -> str:
    return _discovery().get("authorization_endpoint", _AUTH_URL)


def _token_endpoint() -> str:
    return _discovery().get("token_endpoint", _TOKEN_URL)


def _revoke_endpoint() -> str:
    return _discovery().get("revocation_endpoint", _REVOKE_URL)
...
def _post_token(data: dict, _attempt: int = 0) -> Optional[dict]:
    try:
        resp = requests.post(
            _token_endpoint(), data=data,
            headers={"Authorization": _basic_auth(),
                     "Accept": "application/json",
                     "Content-Type": "application/x-www-form-urlencoded"},
            timeout=_HTTP_TIMEOUT,
        )
        # Retry once on a transient server error (5xx) — but NOT on 4xx such as
        # invalid_grant, where retrying would be pointless or harmful.
        if resp.status_code >= 500 and _attempt == 0:
            return _post_token(data, _attempt + 1)
        if not resp.ok:
            log.error("QBO token request %s (tid=%s): %s",
                      resp.status_code, _tid(resp), resp.text[:300])
            return None
        return resp.json()
    except requests.RequestException as e:
        if _attempt == 0:                      # retry once on a network blip
            return _post_token(data, _attempt + 1)
...
def _tid(resp) -> str:
    """QuickBooks stamps every response with an `intuit_tid` header — capturing it
    in our error logs lets Intuit support trace a specific failed call."""
    try:
        return resp.headers.get("intuit_tid", "")
    except Exception:
        return ""
...
def company_info() -> Optional[dict]:
    ...
    if resp is None or not resp.ok:
        if resp is not None:
            log.error("QBO companyinfo %s (tid=%s): %s",
                      resp.status_code, _tid(resp), resp.text[:300])
        return None
...
def disconnect() -> bool:
    ...
    with _lock():
        ...
        if rt and is_configured():
            try:
                requests.post(
                    _revoke_endpoint(), json={"token": rt},
                    headers={"Authorization": _basic_auth(),
                             "Accept": "application/json",
                             "Content-Type": "application/json"},
                    timeout=_HTTP_TIMEOUT,
...
# Cache {custom-field-name(lower) -> DefinitionId} per env, learned from a recent
# invoice, so we can fill the "Project"/"Job No" sales-form fields by name.
_cf_cache: dict = {}


def _custom_field_map() -> dict:
    """Map sales-form custom-field name (lowercased) -> DefinitionId, read from a
    recent invoice and cached 1h per env. Empty if none/unreadable."""
    env = _env()
    cached = _cf_cache.get(env)
    if cached and time.time() - cached["at"] < 3600:
        return cached["map"]
    m: dict = {}
    qr = _query("select * from Invoice maxresults 1")
    if qr:
        invs = qr.get("Invoice", [])
        if invs:
            for cf in invs[0].get("CustomField", []):
                name = (cf.get("Name") or "").strip().lower()
                did = cf.get("DefinitionId")
                if name and did:
                    m[name] = did
    _cf_cache[env] = {"map": m, "at": time.time()}
    return m
...
def create_invoice(customer_id: str, item_id: str, amount, *,
    ...
    QBO customer + service item. Non-taxable. `custom_fields` is {name: value}
    matched to the company's sales-form custom fields by name (e.g. "Job No",
    "Project"). Returns {ok: True, invoice_id, doc_number, total} or
    {ok: False, error}.
    """
    ...
    if custom_fields:
        cf_map = _custom_field_map()
        cfs = []
        for name, value in custom_fields.items():
            if value in (None, ""):
                continue
            did = cf_map.get(name.strip().lower())
            if did:
                # Legacy sales-form custom fields cap StringValue at 31 chars.
                cfs.append({"DefinitionId": did, "Name": name,
                            "Type": "StringType", "StringValue": str(value)[:31]})
        if cfs:
            body["CustomField"] = cfs

    resp = _request("POST", "invoice", json_body=body)
    if resp is None:
        return {"ok": False, "error": "not connected to QuickBooks / network error"}
    if not resp.ok:
        log.error("QBO invoice create %s (tid=%s): %s",
                  resp.status_code, _tid(resp), resp.text[:400])
        return {"ok": False,
                "error": f"QuickBooks {resp.status_code}: {resp.text[:400]}",
                "intuit_tid": _tid(resp)}
...
def attach_file(invoice_id: str, filename: str, content: bytes,
    ...
    access, realm = auth
    metadata = {
        "AttachableRef": [{"EntityRef": {"type": "Invoice", "value": str(invoice_id)}}],
        "FileName": filename,
    ...
    try:
        resp = requests.post(
            f"{_api_base()}/v3/company/{realm}/upload",
            headers={"Authorization": f"Bearer {access}", "Accept": "application/json"},
            params={"minorversion": _MINORVERSION},
            files=files, timeout=60,
        )
    except requests.RequestException as e:
        log.error("QBO attach error for %s: %s", filename, e)
        return {"ok": False, "error": str(e)}
    if not resp.ok:
        log.error("QBO attach %s %s (tid=%s): %s",
                  filename, resp.status_code, _tid(resp), resp.text[:300])
        return {"ok": False, "error": f"QuickBooks {resp.status_code}"}
    return {"ok": True}
