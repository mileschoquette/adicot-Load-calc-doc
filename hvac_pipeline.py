...
class ProjectConfig:
    ...
    bldg_exhaust_all_toilet: bool = False      # M40 — True = zero out + IMC footnote
...
def evaluate_rule(rule: str | None, area_ft2: float, ceiling_ft: float,
                  toilet_exh_cfm: float) -> float | None:
...
class ComputedZone:
    ...
    heating_btuh: float
    tons: float
    supply_cfm: float          # = tons * cfm_per_ton (matches workbook display)
    return_cfm: float          # same as supply by default
...
def _resolve_exh(rv: RoomVent, area_ft2: float, ceiling_ft: float,
                 cfg: ProjectConfig) -> tuple[str, float | None]:
    """Per-room exhaust. Returns (rate_label, cfm)."""
    canonical = resolve_space_type(rv.room_type)
    override = SPACE_TYPE_OVERRIDES.get(canonical) if canonical else None
    if not override or override["exh"] is None:
        return "", None
    rule = override["exh"]
    # Display label: TOILET resolves to the project's toilet_exhaust_cfm number
    label = str(int(cfg.toilet_exhaust_cfm)) if rule == "TOILET" else rule
...
def compute(report: HVACReport, cfg: ProjectConfig) -> ComputedReport:
    ...
    # Build a system-vent-params lookup by zone name (case-insensitive)
    svp_by_zone: dict[str, SystemVentParams] = {
        p.zone_name.lower(): p for p in report.system_vent_params
    }
    ...
    for r1 in report.rooms_p1:
        ...
        exh_label = ""
        exh_cfm = None
        if override and override["exh"] is not None:
            rule = override["exh"]
            exh_label = str(int(cfg.toilet_exhaust_cfm)) if rule == "TOILET" else rule
    ...
    for lt in report.load_total_system:
        if (lt.cool_total_btuh in (0, None)) and (lt.area_ft2 in (0, None)):
            continue
        tons = tonnage_snap(lt.cool_total_btuh, cfg.enable_45_ton_snap)
        supply_cfm = cfm_from_tons(tons, cfg.cfm_per_ton)

        # Bldg Ventilation OA: prefer Vot (ASHRAE-corrected system intake) from system_vent_params.
        # Falls back to sum of per-room Vbz if no per-zone vent params exist.
        svp = svp_by_zone.get(lt.location.lower())
        if svp is not None and svp.vot_cfm is not None:
            vent_oa = excel_ceiling(svp.vot_cfm, 5)
        else:
            zone_room_cfms = [r.vent_cfm for r in out.rooms if r.zone_name == lt.location]
            vent_oa = excel_ceiling(sum(zone_room_cfms), 5)

        zone_room_exhs = [r.exh_cfm or 0.0 for r in out.rooms if r.zone_name == lt.location]
        raw_exh = excel_ceiling(sum(zone_room_exhs), 5)
        bldg_exh = 0.0 if cfg.bldg_exhaust_all_toilet else raw_exh
...
def build_ventilation_schedule_pdf(computed, report, out_path: Path,
    ...
    # Footer total uses the Vot-based system OA (computed.total_vent_oa_cfm) so it matches
    # the Building Air Balance. Summing per-room Vbz gives a slightly higher number because
    # Vbz is the breathing-zone requirement, while Vot is the corrected system intake.
...
def build_air_balance_pdf(computed, out_path: Path,
    ...
    for z in computed.zones:
        tons_label = f"  ({int(z.tons)}Ton)" if z.tons else ""
        data.append([
            _cell(f"  {z.zone_name}{tons_label}"),
...
def apply_zone_overrides(computed: ComputedReport, overrides: dict) -> ComputedReport:
    """Apply zone display overrides: rename, override tons/supply_cfm, merge zones.

    overrides format: {html_zone_name: {display_name?, tons?, supply_cfm?, merge_with?}}
    ...
    for z in new.zones:
        ...
        if target_name in merged:
            ...
            # tons and supply_cfm will be re-derived below
            m.tons = tonnage_snap(m.cooling_total_btuh)
            m.supply_cfm = cfm_from_tons(m.tons)
    ...
    for html_name, ov in overrides.items():
        ...
        if "tons" in ov:
            z.tons = float(ov["tons"])
            # If supply_cfm not separately overridden, recompute it
            if "supply_cfm" not in ov:
                z.supply_cfm = cfm_from_tons(z.tons)
...
def print_air_balance(computed: ComputedReport) -> None:
    ...
    print("-" * 100)
    for z in computed.zones:
        tons_lbl = f"  ({z.tons:g}Ton)" if z.tons else ""
        name = f"{z.zone_name}{tons_lbl}"[:w_zone - 1]
        print(f"{name:<{w_zone}}"
