...
def parse_exhaust_rule(name: str | None) -> str | None:
    ...
    if not name:
        return None
    # "all" = exhaust 100% of supply air. Accept "ACH:all"/"Exh:all" (mirroring
    # the "(Exh:0.5)" tag style) or "all ACH"/"all Exh". ACH still wins over Exh.
    if (re.search(r'ACH\s*:\s*all\b', name, re.I)
            or re.search(r'\ball\s+ACH\b', name, re.I)):
        return "ALL"
    ...
    if re.search(r'exh', name, re.I):
        ...
        if re.search(r'\d+\s*/\s*\d+', name):
            return "TOILET"
        # "Exh:all" / "all Exh" -> exhaust 100% of supply.
        if (re.search(r'exh\s*:\s*all\b', name, re.I)
                or re.search(r'\ball\s+exh\b', name, re.I)):
            return "ALL"
...
assert parse_exhaust_rule("Paint spray booths (Exh:all)") == "ALL"  # 100% of supply
